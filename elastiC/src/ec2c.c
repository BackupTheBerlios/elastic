 ...
 *
 *   $Id: ec2c.c,v 1.2 2005/03/08 13:40:05 panta Exp $
 * --------------------------------------------------------------------------
...
static int doExecute( const char *name, int argc, char *argv[] )
{
	...
	file = fopen( pkgname, "w" );
	{
		ec_compiler2c_ctxt ctxt;

		ctxt = EcCompiler2CContextCreate();
		EcMainCompile2C( ctxt, package, file );
		EcCompiler2CContextDestroy( ctxt );
	}
	fclose( file );
