...
#
# $Id: autogen.sh,v 1.1 2002/05/30 17:46:05 panta Exp $
...
echo aclocal $aclocalinclude -I build
aclocal $aclocalinclude -I build || exit 1
#echo autoheader
#autoheader || exit 1
echo automake --add-missing --gnu $am_opt
automake --add-missing --gnu $am_opt || exit 1
echo autoconf --localdir=build
autoconf --localdir=build || exit 1
